...
document.querySelector(".food-item-container").innerHTML =
  foodItemConstructor.generateHTML(entrees, userCalories);
...
document.querySelectorAll(".food-select-button").forEach((button) => {
  button.addEventListener("click", () => {
    const foodAmount = document.getElementById(
      `amount-${button.dataset.foodId}`
    ).textContent;
